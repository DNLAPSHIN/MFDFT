...
! December 2020.
!
! Solution of the mean field equations in
! the Canonical ensemblefor (NVT) for a 3D
! lattice gas model in finite length slit
! pore with nearest ! neighbor interactions.
!
...
!----------------------------------------------------------------
      ...
      t(10) = "Fe"
      ! open input file
      open(1,file='mft_nvt.dat',status='unknown')
      ...
      read(1,*) ides
      read (1,*) nrho,rho0,drho,rhos
      ! nrho - number of density values
      ! rho0 - initial density
      ! drho - density increment
      read (1,*) cp0
      ! cp0 - initial guess for chemical potential
...
!----------------------------------------------------------------
      ...
      ! loop for adsorption and desorption isotherms
      !do iads=1,ides
          ! loop over density
          do irho = 1, nrho
            rho = rho0+(irho-1)*drho
          do i=1,nsite
              is=ilist(i)
              if(ieta(is).eq.1) then
              ! Compute nearest neighbor sum for rhoj
                  ! initialize solution for adsorption, rhos=0.999
                  if (drho.gt.0.0) then
                    if (iz(is).le.16) then
                    if (ix(is).ge.1) then
                    rhoold(is)=rhos
                  ...
                  ! initialize solution for desorption, rhos=0.000
                    if((iz(is).eq.(mz/2)).or.(iz(is).eq.(mz/2+1)))then
                    if ((ix(is).eq.1).or.(ix(is).eq.mx)) then
                    rhoold(is)=rhos
                    end if
                    end if
                  end if
...
1000      iter = iter + 1
          ...
          gp1 = sumgp1+sumh01
          dcp = tstar*log(rho/rhoav)
          cp = cp+dcp
          ! check for error
          ! if(sumsq.gt.error) go to 1000
          if(abs(dcp/cp).gt.error) go to 1000
          ...
          write(2,'(5g16.8)') tstar,cp,exp((cp+3.0d0)/tstar),rhoav,gp
          write(4,'(5g16.8)') tstar,cp,exp((cp+3.0d0)/tstar),rhoav1,gp1
          ...
          write(3,*) psite         ! the first line in rho*.dat files
...
!----------------------------------------------------------------
      ...
      common/density/ rhoold(1000000),rhonew(1000000)
      common/walls/ ieta(1000000),phi(1000000)
      integer, dimension(3090) :: PSD1, PSD2
